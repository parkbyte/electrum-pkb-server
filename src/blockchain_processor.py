...
class BlockchainProcessor(Processor):
    ...
    def do_catch_up(self):
        ...
        while not self.shared.stopped():
            ...
            if self.shared.paused():
                print_log("parkbyted is responding")
                self.shared.unpause()
            time.sleep(10)
    ...
    def bitcoind(self, method, params=[]):
        ...
        while True:
            try:
                ...
                print_log("cannot reach parkbyted...")
                self.wait_on_bitcoind()
            else:
                r = loads(respdata)
                if r['error'] is not None:
                    if r['error'].get('code') == -28:
                        print_log("parkbyted still warming up...")
                        self.wait_on_bitcoind()
                        continue
    ...
    def getfullblock(self, block_hash):
        ...
        try:
            ...
            logger.error("parkbyted error (getfullblock)",exc_info=True)
            self.shared.stop()
        ...
        for ir in r:
            if ir['error'] is not None:
                self.shared.stop()
                print_log("Error: make sure you run parkbyted with txindex=1; use -reindex if needed.")
